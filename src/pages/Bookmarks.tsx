    ...
    <div className="container mx-auto px-4 py-8 bg-gray-50 min-h-screen">
      ...
      >
        <motion.button
          ...
          className={`px-4 py-3 rounded-lg font-medium flex items-center space-x-2 transition-all duration-200 shadow-md ${activeTab === 'Paper'
        ...
        >
          ...
          {paperBookmarks.length > 0 && (
            <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${activeTab === 'Paper' ? 'bg-white text-blue-600' : 'bg-blue-100 text-blue-600'
              }`}>
        ...
        <motion.button
          ...
          className={`px-4 py-3 rounded-lg font-medium flex items-center space-x-2 transition-all duration-200 shadow-md ${activeTab === 'Resource'
        ...
        >
          ...
          {resourceBookmarks.length > 0 && (
            <span className={`ml-2 px-2 py-0.5 text-xs rounded-full ${activeTab === 'Resource' ? 'bg-white text-indigo-600' : 'bg-indigo-100 text-indigo-600'
              }`}>
        ...
        {currentBookmarks.length === 0 ? (
          ...
          >
            {currentBookmarks.map((bookmark: Bookmark, index) => (
              ...
              >
                <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${getGradient(bookmark)}`}></div>
                <div className="p-6 h-full flex flex-col justify-between">
                  <div>
                    <div className="flex items-center mb-3">
                      <span className="text-2xl mr-2">{getBookmarkIcon(bookmark)}</span>
                      <h3 className="text-xl font-semibold text-gray-800">{bookmark.title}</h3>
                    </div>
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <motion.a
                      ...
                      className={`text-white px-4 py-2.5 rounded-md inline-block transition-all duration-200 shadow-md ${bookmark.type === 'Paper'
                    ...
                    </motion.a>
                    {
                      deletingBookmark && bookmark.id === bookmarkToDelete
                        ...
                        : <motion.button
                          whileHover={{ scale: 1.2, backgroundColor: 'rgba(254, 202, 202, 0.2)' }}
                          whileTap={{ scale: 0.8 }}
