  ...
  const handleSubjectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedSubject =
      formData.branch === "IT" && formData.year !== "FE"
        ? IT_SUBJECTS["2019Pattern"][formData.year as "SE" | "TE" | "BE"].find(
            (subject) => subject.name === e.target.value
          )
        : FE_SUBJECTS["2019Pattern"].find(
            (subject) => subject.name === e.target.value
          );
    if (selectedSubject) {
      setFormData({
  ...
  if (!isAdmin) {
    return (
      <div className="text-center py-8">
        Please log in as admin to add resources.
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <select
          ...
          required
        >
          {semesterMap[formData.branch === "FE" ? "FE" : formData.year]?.map(
            (sem) => (
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setFormData({
              ...formData,
              pattern: e.target.value as Resource["pattern"],
            })
          }
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
          required
