...
const TaskBoard = ({ boards }: TaskBoardProps) => {
  ...
  const { tasks, lists, selectedBoardId, loading } = useSelector(
  ...
  const [activeListenerBoardId, setActiveListenerBoardId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (
      activeListenerBoardId !== selectedBoardId &&
      activeListenerBoardId !== null
    ) {
      dispatch(setLists([]));
      dispatch(setTasks([]));
    }
      ...
      <div className="w-full">
        ...
        >
          ...
          <div
            ...
            } transition-all duration-300 z-10 mt-1 w-56 bg-white border border-gray-200 rounded-md shadow-lg`}
          >
            <div className="py-1">
              {boards.map((board) => (
                <button
        ...
        {selectedBoardId && (
          <div className="flex p-4 overflow-x-auto pb-6 h-full">
            <div className="flex space-x-4 md:space-x-6 pb-4 md:pb-0 snap-x snap-mandatory">
              {lists
                .filter((list) => list.boardId === selectedBoardId)
                .map((list) => (
